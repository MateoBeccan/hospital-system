...
import com.mycompany.hospital.service.PaisQueryService;
import com.mycompany.hospital.service.PaisService;
import com.mycompany.hospital.service.criteria.PaisCriteria;
...
/**
 * REST controller for managing {@link com.mycompany.hospital.domain.Pais}.
 */
@RestController
...
public class PaisResource {
    ...
    public ResponseEntity<PaisDTO> createPais(@Valid @RequestBody PaisDTO paisDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/pais/" + paisDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, paisDTO.getId().toString()))
            .body(paisDTO);
    }
    ...
    public ResponseEntity<List<PaisDTO>> getAllPaises(
        ...
        LOG.debug("REST request to get Paises by criteria: {}", criteria);
    ...
    /**
     * {@code GET  /pais/count} : count all the paises.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countPaises(PaisCriteria criteria) {
