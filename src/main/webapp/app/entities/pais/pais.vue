<template>
  <div>
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link :to="{ name: 'PaisCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-pais">
            ...
            <span>Crear nuevo Pais</span>
          </button>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && paises?.length === 0">
      <span>Ningún Pais encontrado</span>
    </div>
    <div class="table-responsive" v-if="paises?.length > 0">
      <table class="table table-striped" aria-describedby="paises">
        <thead>
          <tr>
            ...
            <th scope="col" @click="changeOrder('codigoIso')">
              <span>Codigo Iso</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'codigoIso'"></jhi-sort-indicator>
            </th>
        ...
        <tbody>
          <tr v-for="pais in paises" :key="pais.id" data-cy="entityTable">
            <td>
              <router-link :to="{ name: 'PaisView', params: { paisId: pais.id } }">{{ pais.id }}</router-link>
            </td>
            <td>{{ pais.nombre }}</td>
            <td>{{ pais.codigoIso }}</td>
            <td>{{ pais.fechaAlta }}</td>
            <td>{{ pais.fechaBaja }}</td>
            <td>{{ pais.activo }}</td>
            <td class="text-end">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #title>
        <span id="hospitalApp.pais.delete.question" data-cy="paisDeleteDialogHeading">Confirmar operación de borrado</span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-pais-heading">¿Seguro que quiere eliminar Pais {{ removeId }}?</p>
      </div>
      <template #footer>
        <div>
